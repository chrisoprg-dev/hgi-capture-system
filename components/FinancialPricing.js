...
function FinancialPricing({ sharedCtx={} }) {
  ...
  const exportFinancialPackage = async () => {
    ...
    if (!hasContent) { alert('Generate at least one analysis first (Market Rates, PTW, Eval Model, or Cost Narrative).'); return; }
 ...
 + Math.round(n).toLocaleString();
    ...
    // Build cost buildup summary as text for the doc
    const laborSummary = laborRows.map(r => {
      const c = calcRow(r);
      return `- ${r.cat}: ${r.hours} hrs @ ${fmt2(c.loaded)}/hr loaded = ${fmt2(c.total)}`;
    }).join('\n');
    const periodSummary = periods.map((p, i) => `- ${p.label}: ${fmt2(periodTotal(p, i))}`).join('\n');
    const odcSummary = odcs.map(o => `- ${o.desc}: ${fmt2(parseFloat(o.amount||0))}`).join('\n');

    const builtupContent = `## COST BUILDUP SUMMARY\n\n### Labor Categories\n${laborSummary}\n\n**Total Labor: ${fmt2(totalLabor())}**\n\n### Other Direct Costs (ODCs)\n${odcSummary}\n\n**Total ODCs: ${fmt2(totalODC())}**\n\n### Period of Performance Pricing\n${periodSummary}\n\n**Total Contract Value: ${fmt2(contractTotal())}**\n\n**Profit Margin: ${profitMargin()}%** (Target: ${intel.hgiMarginTarget}%)\n${recommendedPrice ? '\n**System Recommended Win Price: ' + fmt2(recommendedPrice) + '**' : ''}`;

    // Combine all available sections
    ...
    try {
      const resp = await fetch('/api/export-module', {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            'Agency': agency,
            'Contract Value': intel.estimatedValue || fmt2(contractTotal()),
            'Contract Years': intel.contractYears || periods.length + ' periods',
            'Contract Type': intel.contractType || '',
            'Incumbent': intel.incumbent || 'Unknown',
            'Price Weight': intel.pricingWeight ? intel.pricingWeight + '%' : '',
            'HGI Buildup': fmt2(contractTotal()),
            'Profit Margin': profitMargin() + '%',
            'Win Price': recommendedPrice ? fmt2(recommendedPrice) : 'Not yet modeled',
            'Sections Included': [marketRates && 'Market Rates', ptwAnalysis && 'PTW Analysis', evalResult && 'Eval Model', costNarrative && 'Cost Narrative'].filter(Boolean).join(', ') || 'Cost Buildup only'
          }
        })
      });
