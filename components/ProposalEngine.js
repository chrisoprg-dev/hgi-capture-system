...
function ProposalEngine({ sharedCtx={}, defaultSection="executive_summary" }) {
  ...
  const buildPrompt = (sLabel, activeRfp, kbInjection) => {
    const ctx = localCtx || sharedCtx;
    const decomp = ctx.decomposition ? "\n\nRFP DECOMPOSITION:\n" + ctx.decomposition.slice(0, 4000) : "";
    const research = ctx.research ? "\n\nCOMPETITIVE RESEARCH:\n" + ctx.research.slice(0, 2500) : "";
    const brief = ctx.execBrief ? "\n\nEXECUTIVE BRIEF SUMMARY:\n" + ctx.execBrief.slice(0, 2000) : "";
    const kb = kbInjection ? "\n\nHGI INSTITUTIONAL KNOWLEDGE BASE:\n" + kbInjection.slice(0, 5000) : "";
    return "Write a complete, detailed " + sLabel + " section for HGI proposal.\nRFP Context: " + (activeRfp||"General disaster recovery TPA services") + "\nAdditional Context: " + context + decomp + brief + research + kb + "\n\nCRITICAL INSTRUCTIONS:\n1. EVALUATION CRITERIA: Search the RFP text above for evaluation criteria, scoring weights, or point values. Structure this section to directly address the highest-weighted criteria first. If the RFP specifies factors like Technical (30pts), Experience (25pts), Past Performance (20pts), Staffing (15pts), Pricing (10pts) — write to maximize points on the criteria this section maps to.\n2. RFP POSITION TITLES: If this section involves staffing or pricing, use ONLY the exact position titles specified in the RFP. Do not substitute or add positions the RFP did not request. For rates, check the KB for comparable reference rates but adapt them to this specific RFP.\n3. AGENCY-SPECIFIC: Reference the specific agency by name throughout. Do not write generic content — tailor every paragraph to this agency and this scope.\n4. PAST PERFORMANCE: Use only verified HGI past performance from the knowledge base. Match past performance examples to the specific requirements of this RFP.\n5. Be thorough and specific. At least 600 words.";
  };
  ...
  return (
    <div>
      ...
      {React.createElement(OpportunitySelector,{pipeline:pl.pipeline,selected:pl.selected,onSelect:function(opp){pl.select(opp);setProposalDraft({});store.set("proposalDraft",{});setResult("");setAutoProgress([]);var oppCtx={rfpText:opp.rfp_text||"",title:opp.title||"",agency:opp.agency||"",vertical:opp.vertical||"disaster_recovery",decomposition:opp.scope_analysis||"",execBrief:opp.capture_action||"",research:opp.research_brief||""};setLocalCtx(oppCtx);if(opp.rfp_text){setRfpText(opp.rfp_text);setContext(opp.title+' — '+opp.agency);}if(opp.staffing_plan&&opp.staffing_plan.indexOf('=== ')===0){var saved={};var parts=opp.staffing_plan.split(/\n=== /);parts.forEach(function(p){var nl=p.indexOf('\n');if(nl>0){var k=p.slice(0,nl).replace(/^=== /,'').replace(/ ===/,'').toLowerCase().replace(/ /g,'_');var v=p.slice(nl+1).trim();if(k&&v)saved[k]=v;}});if(Object.keys(saved).length>0){setProposalDraft(saved);store.set('proposalDraft',saved);setActiveView('workspace');}}},loading:pl.loading,label:"SELECT OPPORTUNITY FOR PROPOSAL"})}
