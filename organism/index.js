...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/health') {
    res.writeHead(200);
    res.end(JSON.stringify({ status: 'alive', uptime_seconds: Math.floor(process.uptime()), timestamp: new Date().toISOString(), version: 'V2.8.0-thirtyeight-agents', agents_active: 38 }));
    return;
  }
...
// ── AGENT 38: HUNTING AGENT (real scraper — goes and gets them) ────
async function agentHunting(state, ctx) {
  log('HUNTING AGENT: actively scraping procurement portals...');

  var newOpps = [];
  var existingIds = new Set(state.pipeline.map(function(o) { return (o.id||'').toLowerCase(); }));
  var existingTitles = state.pipeline.map(function(o) { return (o.title||'').toLowerCase().slice(0,40); });

  function isDupe(title) {
    var t = (title||'').toLowerCase().slice(0,40);
    return existingTitles.some(function(e) {
      var matches = 0;
      var words = t.split(' ').filter(function(w) { return w.length > 3; });
      words.forEach(function(w) { if (e.includes(w)) matches++; });
      return words.length > 0 && matches / words.length > 0.5;
    });
  }

  // ── SOURCE 1: SAM.gov API ──────────────────────────────────────
  var samKeywords = ['disaster recovery','program management','grant administration','claims administration','housing authority','workforce development','TPA','third party administrator','FEMA','CDBG','hazard mitigation'];
  var samFound = 0;
  for (var k = 0; k < samKeywords.length; k++) {
    try {
      var samUrl = 'https://api.sam.gov/opportunities/v2/search?api_key=DEMO_KEY&q=' + encodeURIComponent(samKeywords[k]) + '&postedFrom=' + getDateDaysAgo(7) + '&postedTo=' + getToday() + '&ptype=o,p,k&active=true&limit=10';
      var samResp = await fetch(samUrl);
      if (samResp.ok) {
        var samData = await samResp.json();
        var opps = (samData.opportunitiesData || []);
        for (var i = 0; i < opps.length; i++) {
          var o = opps[i];
          if (!isDupe(o.title)) {
            newOpps.push({ title: o.title, agency: o.fullParentPathName || o.organizationCode || 'Federal', source: 'sam_gov', source_url: 'https://sam.gov/opp/' + o.opportunityId, description: (o.description||'').slice(0,500), due_date: o.responseDeadLine, vertical: null });
            samFound++;
          }
        }
      }
    } catch(e) { log('HUNTING SAM error: ' + e.message); }
  }
  log('HUNTING: SAM.gov found ' + samFound + ' candidates');

  // ── SOURCE 2: Grants.gov API ───────────────────────────────────
  var grantsKeywords = ['disaster recovery','housing','workforce','program administration','claims','grant management'];
  var grantsFound = 0;
  try {
    var grantsResp = await fetch('https://api.grants.gov/v1/api/search2', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ keyword: grantsKeywords.join(' OR '), rows: 25, oppStatuses: 'forecasted|posted', startRecordNum: 0 })
    });
    if (grantsResp.ok) {
      var grantsData = await grantsResp.json();
      var grants = (grantsData.data && grantsData.data.oppHits) ? grantsData.data.oppHits : [];
      for (var g = 0; g < grants.length; g++) {
        var gr = grants[g];
        if (!isDupe(gr.oppTitle)) {
          newOpps.push({ title: gr.oppTitle, agency: gr.agencyName || 'Federal', source: 'grants_gov', source_url: 'https://grants.gov/search-grants?oppNumber=' + gr.number, description: (gr.synopsis||'').slice(0,500), due_date: gr.closeDate, vertical: 'grant' });
          grantsFound++;
        }
      }
    }
  ...
  // ── SOURCE 3: LaPAC API ────────────────────────────────────────
  var lapacKeywords = ['program management','professional services','disaster','housing','workforce','grant','TPA','claims','consulting'];
  var lapacFound = 0;
  for (var lk = 0; lk < lapacKeywords.length; lk++) {
    try {
      var lapacResp = await fetch('https://wwwcfts.doa.la.gov/lascts/rest/solicitations?keyword=' + encodeURIComponent(lapacKeywords[lk]) + '&status=OPEN&rows=10', { headers: { 'Accept': 'application/json' } });
      if (lapacResp.ok) {
        var lapacData = await lapacResp.json();
        var lapacOpps = Array.isArray(lapacData) ? lapacData : (lapacData.solicitations || lapacData.results || []);
        for (var lp = 0; lp < lapacOpps.length; lp++) {
          var lo = lapacOpps[lp];
          var title = lo.title || lo.solicitationTitle || lo.description || '';
          if (title && !isDupe(title)) {
            newOpps.push({ title: title, agency: lo.agency || lo.agencyName || 'Louisiana Agency', source: 'lapac', source_url: lo.url || 'https://wwwcfts.doa.la.gov', description: (lo.description||title).slice(0,500), due_date: lo.dueDate || lo.closingDate, vertical: null });
            lapacFound++;
          }
        }
      }
  ...
  log('HUNTING: Total candidates found: ' + newOpps.length + '. Now scoring with full OPI model...');

  if (newOpps.length === 0) {
    await storeMemory('hunting_agent', null, 'hunting,no_new_opps', 'HUNTING: No new qualifying opportunities found this session. Sources checked: SAM.gov, Grants.gov, LaPAC.', 'analysis');
    return { agent: 'hunting_agent', chars: 50, new_opps: 0 };
  }

  // ── SCORE EACH CANDIDATE WITH FULL OPI MODEL ──────────────────
  var qualified = [];
  for (var c = 0; c < Math.min(newOpps.length, 15); c++) {
    var cand = newOpps[c];
    try {
      var scorePrompt = HGI +
        ...
        '\n\nORGANISM CONTEXT:\n' + ctx.memText.slice(0,400) +
        '\n\nEVALUATE THIS OPPORTUNITY FOR HGI. Respond in JSON only. No markdown. Format:\n' +
        '{"opi":NUMBER_0_TO_100,"vertical":"disaster|tpa|workforce|housing|construction|grant|federal|general|FILTER","capture_action":"GO or NO-BID","why":"one sentence","estimated_value":"dollar range or unknown"}\n' +
        'Use vertical=FILTER to reject clearly irrelevant opportunities (physical construction, healthcare, IT, education, etc).';
      ...
      var scoreText = (scoreResp.content||[]).filter(function(b){return b.type==='text';}).map(function(b){return b.text;}).join('').replace(/json|/gi,'').trim();
      ...
      log('HUNTING: QUALIFIED ' + cand.title.slice(0,40) + ' OPI:' + score.opi + ' ' + score.vertical);

      // Write directly to pipeline
      var newId = cand.source + '-hunt-' + Date.now() + '-' + Math.random().toString(36).slice(2,6);
      ...
      qualified.push({ title: cand.title, opi: score.opi, vertical: score.vertical, agency: cand.agency });

    } catch(e) { log('HUNTING: score error on ' + cand.title.slice(0,30) + ': ' + e.message); }
  }

  log('HUNTING AGENT COMPLETE: ' + qualified.length + ' new opportunities added to pipeline');
  if (qualified.length > 0) {
    log('NEW PIPELINE ADDITIONS:');
    qualified.forEach(function(q) { log('  + OPI:' + q.opi + ' | ' + q.vertical + ' | ' + q.title.slice(0,50)); });
  }

  await storeMemory('hunting_agent', null,
    'hunting,new_opportunities,pipeline_growth',
    'HUNTING AGENT: Found ' + newOpps.length + ' candidates across SAM.gov/Grants.gov/LaPAC. Qualified and added ' + qualified.length + ' new opportunities to pipeline.\n' + qualified.map(function(q){return 'OPI:' + q.opi + ' ' + q.title.slice(0,50);}).join('\n'),
    'analysis'
  );

  return { agent: 'hunting_agent', chars: 200, new_opps: qualified.length };
}
