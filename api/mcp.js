...
const TOOLS = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        filename: { type: 'string', description: 'File path e.g. api/intake.js or components/Dashboard.js' }
      },
      required: ['filename']
    }
  },
  ...
  switch (name) {
    ...
    case 'modify_system': {
      ...
      if (isLarge) {
        ...
        if (/const \/\/|var \/\/|let \/\//.test(finalContent)) return { error: 'Validation failed: broken replacement' };
      } else {
    ...
    case 'query_database': {
      ...
      return { table, data: await sb(path) };
    }
