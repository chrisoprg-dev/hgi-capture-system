...
export default async function handler(req, res) {
  ...
  try {
    const rawRecord = {
      ...
      rfp_text: incomingRfpText || (stripScripts(raw_html) + " " + description).slice(0, 500000),
  ...
  // ── Claude analysis ──────────────────────────────────────────────────────
  const fullText = (incomingRfpText || (description + " " + stripScripts(raw_html))).slice(0, 30000);
