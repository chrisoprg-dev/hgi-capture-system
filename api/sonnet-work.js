...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (gateVerdict === 'NO-GO') {
      ...
      var proposalSystem = 'You are a senior proposal writer building a winning government proposal. You have the full RFP scope, the current draft, quality gate findings, and HGI confirmed facts. Your job is to produce submission-ready text.\n\nCRITICAL RULES:\n1. If a required RFP section is MISSING or has fewer than 500 words relative to its evaluation point weight, BUILD IT COMPLETELY from the RFP requirements and HGI facts. Do not note the gap — fill it.\n2. Technical Approach is ALWAYS the highest priority. If Section D is absent, thin, or generic, write the entire section from scratch: activation protocol, FEMA PA methodology by category, CDBG-DR approach, compliance infrastructure, agency liaison plan, staffing model. Use specific FEMA terminology, cite actual policy guides (PAPPG, 2 CFR 200), reference HGI actual staff by name and credential.\n3. CONFIRMED HGI REFERENCES ARE FACTS. Never question, flag, or suggest replacing: Paul Rainwater (rainwater97@gmail.com, 225-281-8176), Jeff Haley (jeff.haley@la.gov, 225-330-0036), Pat Forbes (Patrick.Forbes@la.gov, 225-342-1626), Bubba Orgeron (bubbaorgeron@tpsd.org, 985-876-7400), Gregory Harding (gregoryharding@tpsd.org, 985-688-0052). These contacts are verified by HGI. Use them as written.\n4. Write actual proposal text — paragraphs, tables, matrices ready to paste. Never describe what should be written.\n5. Do not fabricate past performance values, contract amounts, or credentials not in the draft or HGI facts.';
      var p = await sonnet(proposalSystem, ctx + '\n\nGATE FINDINGS (address these first, build missing sections completely):\n' + g.slice(0,1000) + '\n\nFor every section below 8/10 or missing entirely: write the complete improved or new section text. Prioritize by eval point weight. Technical Approach (30 pts) first if missing or weak.', 4000);
