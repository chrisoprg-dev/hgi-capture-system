...
export default async function handler(req, res) {
  ...
  try {
    financialAnalysis = await claudeCall(
      ...
      '2. STAFFING PLAN — Read the RFP text carefully. If the RFP specifies required positions or a pricing exhibit with named roles, build the staffing plan using ONLY those RFP-specified positions mapped to the closest HGI rate card position. Do NOT invent a generic staffing model. Match the RFP structure exactly. For each role show: RFP Position Title, Mapped HGI Title, HGI Hourly Rate, estimated hours per month, monthly cost, and justification. If the RFP does not specify positions, then propose a lean staffing model appropriate for the contract size.\n\n' +
