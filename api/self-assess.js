...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isDebug) {
      var debugData = JSON.stringify({ debug: true, pipeline_active: activeOpps.length, outcomes: allOutcomes.length, hunts: recentHunts.length, qg: qualityGateRuns.length, kb: kbDocs.length, kb_processed: kbDocs.filter(function(d){return d.status==='processed';}).length, stale: staleHighOpi.length, organism: agentSummary, agents: agentDetails.length, opi_calibration: opiAccuracy });
