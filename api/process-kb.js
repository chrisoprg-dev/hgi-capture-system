...
export default async function handler(req, res) {
  ...
  var findResp = await fetch(SUPABASE_URL + "/rest/v1/knowledge_documents?or=(status.eq.pending_extraction,status.eq.error,status.eq.processing,status.eq.uploaded)&order=uploaded_at.desc&limit=1&select=id,filename,storage_path,mime_type,file_type,status", {
    headers: dbH
  });
