...
export default async function handler(req, res) {
  ...
  var findResp = await fetch(SUPABASE_URL + "/rest/v1/knowledge_documents?chunk_count=eq.0&storage_path=not.is.null&status=neq.error&order=uploaded_at.desc&limit=1&select=id,filename,storage_path,mime_type,file_type,status", {
    headers: dbH
  });
