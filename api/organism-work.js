...
async function agentPipelineScanner(activeOpps, memText) {
  ...
  var web = ''; // gated — searches only via discovery agent
  var _skip = agencies + ' RFP amendment addendum extension deadline change 2026');
...
async function agentKb(memText) {
  var web = await webSearch('HGI Global Hammerman Gainer disaster recovery Louisiana program management contracts awards past performance 2020 2021 2022 2023 2024');
  var webCtx = (web && web.length > 30) ? ('\nWEB HGI DATA:\n' + web.slice(0,1200)) : '';
